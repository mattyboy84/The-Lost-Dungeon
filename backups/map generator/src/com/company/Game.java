...
//https://www.youtube.com/watch?v=iOFclU4zURE
    ...
    public void start(Stage stage) {
        ...
        Tooltip.install(mapItem, new Tooltip("Buy to reveal the dungeon layout"));
    ...
    private void bossRoom(Group group, Scene scene, ImageView chest, ImageView bossHealthBar, Rectangle bossHealthBarREC, ImageView hearts, Label score, ImageView hero, ImageView boss, Stage stage) {
        ...
        if (k == 0) {
            Bossroomtimeline = new Timeline(new KeyFrame(Duration.seconds(0.05), event -> {
                try {
                    ...
                    if (y >= 8) {
                        bossPhase2(group, scene, chest, bossHealthBar, bossHealthBarREC, hearts, score, hero, boss, stage);
                    }
                } catch (Exception e) {
    ...
    private void bossPhase2(Group group, Scene scene, ImageView chest, ImageView bossHealthBar, Rectangle bossHealthBarREC, ImageView hearts, Label score, ImageView hero, ImageView boss, Stage stage) {//phase 2 of the boss fight
        Random rand = new Random();
            ...
            case "walking":
                if (l == 0 && walking == 1) {
                    walkSounds = new Timeline(new KeyFrame(Duration.seconds(0.4), event -> {
                        l = 1;
                        File f = new File("src\\game\\sound\\walkingSound.wav");
    ...
    private void inventoryHider(ImageView controls, ImageView inventoryPIC, ImageView selected, Label itemName, Label itemType, Label itemDescription, Label itemEffect) {
    ...
    private void inventoryManager(ImageView inventoryPIC, ImageView selected, Label itemName, Label itemType, Label itemDescription, Label itemEffect) {
        if (!inventoryPIC.isVisible()) {
            itemInformation(selected, itemName, itemType, itemDescription, itemEffect);
            ...
            scene.setOnKeyPressed(event -> {
                switch (event.getCode()) {
                    ...
                    case SPACE:
                        ...
                        AttackHandler(score, group, translate, boss, link_sword_down, link_sword_left, link_sword_right, link_sword_up, link_staff_down, link_staff_left, link_staff_right, link_staff_up, sword_down, sword_left, sword_right, sword_up, usedSword, bossDoor_closed, bossDoor_open, bossDoor, bossHealthBar, bossHealthBarREC, coinNumber, link_running_down_1, link_running_left_1, link_running_right_1, link_running_up_1, hero, link_running_down_2, link_running_left_2, link_running_right_2, link_running_up_2);
                        inventoryHider(controls, inventoryPic, selected, itemName, itemType, itemDescription, itemEffect);
                        ScoreBoardHider(scoreBoard, Labels);
                        break;
                    case TAB:
                        inventoryManager(inventoryPic, selected, itemName, itemType, itemDescription, itemEffect);
                        itemInformation(selected, itemName, itemType, itemDescription, itemEffect);
                        break;
                    case EQUALS:
                        if (musicVolume < 10) {
                    ...
                    case D:
                        ...
                        break;
                    case E:
                        itemInteraction(itemId, group, scene, playerName, hero, indicator, chest, chest_open, staff, attackBoost, coinIcon_staff, coinIcon_attackBoost, hearts, inventoryPic, selected, coinNumber_staff, coinNumber_attackBoost, coinNumber, Labels, mapItem, coinIcon_mapItem, coinNumber_mapItem, stage, score, itemName, itemType, itemDescription, itemEffect);
                        break;
                    case I:
                        if (!controls.isVisible()) {
    ...
    private void itemInformation(ImageView selected, Label itemName, Label itemType, Label itemDescription, Label itemEffect) {
        for (int i1 = 0; i1 < inventory.size(); i1++) {//checks all items
            ...
            if (selected.getBoundsInParent().intersects(item.getImageViewIMG().getBoundsInParent()) && item.getImageViewIMG().isVisible() && item.getImageViewIMG().getOpacity() > 0) {
                ...
                //itemEffect.relocate(1030, 430);
                //itemEffectMover(itemEffect,itemDescription);
    ...
    /*
        private void itemEffectMover(Label itemEffect, Label itemDescription) {
            if (itemEffect.getBoundsInParent().intersects(itemDescription.getBoundsInParent())){
                itemEffect.setLayoutY(itemEffect.getLayoutY()+40);
                itemEffectMover(itemEffect,itemDescription);
    ...
    */
    ...
    private void scoreSaver(Label[][] Labels, String playerName) {
        String name;
    ...
    private void itemInteraction(int itemId, Group group, Scene scene, String playerName, ImageView hero, ImageView indicator, ImageView chest, Image chest_open, ImageView staff, ImageView attackBoost, ImageView coinIcon_staff, ImageView coinIcon_attackBoost, ImageView hearts, ImageView inventoryPIC, ImageView selected, Label coinNumber_staff, Label coinNumber_attackBoost, Label coinNumber, Label Labels[][], ImageView mapItem, ImageView coinIcon_mapItem, Label coinNumber_mapItem, Stage stage, Label score, Label itemName, Label itemType, Label itemDescription, Label itemEffect) {
        ...
        if (hero.getBoundsInParent().intersects(attackBoost.getBoundsInParent()) && attackBoost.isVisible() && CoinNumber >= 5) {//attack boost item
            ...
            for (Inventory anInventory : inventory) {
                anInventory.getImage().setOnMouseClicked(event -> usingItems(score, group, scene, anInventory, hearts, stage, selected, itemName, itemType, itemDescription, itemEffect));
            }
            if (ii == 5) {
                ii = 0; //puts an imageViewIMG onto the next row
                jj++; //next column
            }
            inventory.get(b).getImage().relocate(642 + (69 * ii), 314 + (69 * jj));
        ...
        if (chest.isVisible() && hero.getBoundsInParent().intersects(chest.getBoundsInParent())) {
            if (f == 0) {
                ...
                soundManager();
                //triforce
                ImageView triforce = new ImageView("file:src\\game\\triforce.png");
                triforce.relocate(1620, 0);
                group.getChildren().add(triforce);
                scoreSaver(Labels, playerName);
            }
        }
        if (!inventoryPIC.isVisible()) {
            ...
            for (Inventory anInventory : inventory) {
                if (selected.getBoundsInParent().intersects(anInventory.getImage().getBoundsInParent()) && anInventory.getImage().getOpacity() > 0 && anInventory.getImage().isVisible()) {
                    usingItems(score, group, scene, anInventory, hearts, stage, selected, itemName, itemType, itemDescription, itemEffect);
                }
            }
        }
    }

    private void usingItems(Label score, Group group, Scene scene, Inventory anInventory, ImageView hearts, Stage stage, ImageView selected, Label itemName, Label itemType, Label itemDescription, Label itemEffect) {
        if (anInventory.getImageViewIMG().getOpacity() > 0 && anInventory.getImageViewIMG().isVisible()) {
            Score = Score - 25;
            score.setText(String.valueOf(Score));
    ...
    private void deathScreen(Group group, Scene scene, Stage stage, String killedBy) {
        ...
        death.setCycleCount(17);  //16 cycles of 0.5 seconds = 8 seconds for the death screen to complete
        death.play();
    }

    private void gameReset(Stage stage, Group group, Scene scene) {
        firstRun = false;
        positionY = 9;
        positionX = 9;
        stage.setScene(scene);
    ...
    private void staffShooter(Label score, Group group, TranslateTransition translate, ImageView boss, Image bossDoor_open, ImageView bossDoor, Rectangle bossHealthBarREC, Label coinNumber, ImageView hero) {
        try {
    ...
    private void attackingEnemies(Label score, TranslateTransition translate, ImageView boss, Rectangle sword_down, Rectangle sword_left, Rectangle sword_right, Rectangle sword_up, Rectangle usedSword, Image bossDoor_closed, Image bossDoor_open, ImageView bossDoor, ImageView bossHealthBar, Rectangle bossHealthBarREC, Label coinNumber) {
        try {
            if (activeBoss) {
    ...
    private void enemyMover(Label score, TranslateTransition translate, ImageView hearts, int[][] map, ImageView hero, Group group, Scene scene, Stage stage) {
        ...
        switch (attackDirection) {
            ...
            case "down_staff":
                if (hero.getImage() != link_staff_down && hero.getImage() != link_staff_up && hero.getImage() != link_staff_left && hero.getImage() != link_staff_right) {
                    ...
                    if (timepassed7 >= 2) {
                        staffShooter(score, group, translate, boss, bossDoor_open, bossDoor, bossHealthBarREC, coinNumber, hero);
                        timepassed7 = 0;
                        timepassed1 = 0;
                    }
                }
    ...
    private void roomChanger(Group group, Scene scene, TranslateTransition translate, ImageView boss, ImageView indicator, int[][] map, ImageView hearts, ImageView northDoor, ImageView eastDoor, ImageView southDoor, ImageView westDoor, ImageView bossDoor, Image bossDoor_closed, Image bossDoor_open, ImageView chest, ImageView bossHealthBar, Rectangle bossHealthBarREC, Label score, ImageView background, ImageView shopkeeper, ImageView staff, ImageView attackBoost, ImageView coinIcon_staff, ImageView coinIcon_attackBoost, Label coinNumber_attackBoost, Label coinNumber_staff, ImageView mapItem, ImageView coinIcon_mapItem, Label coinNumber_mapItem, ImageView hero, Stage stage) {
        if (hero.getBoundsInParent().intersects(bossDoor.getBoundsInParent()) /*&& bossDoor.getImage()==bossDoor_open*/ && bossDoor.isVisible() && Objects.equals(direction, "up")) {
            ...
            if (v == 0) {
                activeBoss = true;
                bossRoom(group, scene, chest, bossHealthBar, bossHealthBarREC, hearts, score, hero, boss, stage);
                v = 1;
            }
            for (Items image : items) {
        ...
        if (hero.getBoundsInParent().intersects(northDoor.getBoundsInParent()) && northDoor.isVisible() && Objects.equals(direction, "up")) {
            ...
            indicator.relocate(indicator.getLayoutX(), indicator.getLayoutY() - 64);
            hero.relocate((960) - 75, 750);
            Base_Procedure2(score, translate, hero, map, northDoor, eastDoor, southDoor, westDoor, bossDoor, bossDoor_closed, bossDoor_open, background, hearts, shopkeeper, staff, attackBoost, coinIcon_staff, coinIcon_attackBoost, coinNumber_attackBoost, coinNumber_staff, mapItem, coinIcon_mapItem, coinNumber_mapItem, group, scene, stage);
        }
    ...
    private void Base_Procedure2(Label score, TranslateTransition translate, ImageView hero, int[][] map, ImageView northDoor, ImageView eastDoor, ImageView southDoor, ImageView westDoor, ImageView bossDoor, Image bossDoor_closed, Image bossDoor_open, ImageView background, ImageView hearts, ImageView shopkeeper, ImageView staff, ImageView attackBoost, ImageView coinIcon_staff, ImageView coinIcon_attackBoost, Label coinNumber_attackBoost, Label coinNumber_staff, ImageView mapItem, ImageView coinIcon_mapItem, Label coinNumber_mapItem, Group group, Scene scene, Stage stage) {
        doorChanger(map, northDoor, eastDoor, southDoor, westDoor, bossDoor, bossDoor_closed, bossDoor_open);
        enemyChanger();
        backgroundChanger(background);
        enemyMover(score, translate, hearts, map, hero, group, scene, stage);
        itemChanger();
        shopChecker(map, shopkeeper, staff, attackBoost, coinIcon_staff, coinIcon_attackBoost, coinNumber_attackBoost, coinNumber_staff, mapItem, coinIcon_mapItem, coinNumber_mapItem);
    ...
    private static void shopChecker(int[][] map, ImageView shopkeeper, ImageView staff, ImageView attackBoost, ImageView coinIcon_staff, ImageView coinIcon_attackBoost, Label coinNumber_attackBoost, Label coinNumber_staff, ImageView mapItem, ImageView coinIcon_mapItem, Label coinNumber_mapItem) {
