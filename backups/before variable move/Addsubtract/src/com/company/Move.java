...
public class Move extends Application {
    ...
    private void soundManager() {
        switch (sound) {
            case "walking":
                if (l == 0 && walking == 1) {
                    walkSounds = new Timeline(new KeyFrame(Duration.seconds(0.4 * musicVolume), event -> {
                        l = 1;
                        File f = new File("E:\\game\\sound\\walkingSound.wav");
                        Media media = new Media(f.toURI().toString());
                        mplayer = new MediaPlayer(media);
                        mplayer.setVolume(0.10);
    ...
    */
    ...
    private void enemyMover() {
        ...
        if (s == 0) {
            Timeline EnemyMover = new Timeline(new KeyFrame(Duration.seconds(0.3), event -> {
                s = 1;
                number++;
                int currentFloorSUB1 = Integer.parseInt(currentFloor.substring(6));
                currentFloorSUB1--;
                int finalCurrentFloorSUB = currentFloorSUB1;
                if (!(currentFloorSUB1 == 7 || currentFloorSUB1 == 6 || currentFloorSUB1 == 8)) {
