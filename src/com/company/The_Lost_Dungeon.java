...
public class The_Lost_Dungeon extends Application {
    ...
    //shop items should be generated in the items class, always keep at the end of array. pressing E branches into item name - more efficient. YAY
    ...
    ArrayList<Boss> bossARR = new ArrayList<>();
    ...
    public void start(Stage stage) {
        ...
        ImageView hero = new ImageView("file:src\\game\\oldSprites\\link_running_down_1.png");
        //
        ImageView bossIMG = new ImageView("file:src\\game\\Wizard.png");
        ...
        */
        ...
        //BOSS
        bossIMG.relocate(885, 459);
        bossIMG.setVisible(false);
        group.getChildren().add(bossIMG);
        //boss health
        ...
        stage.setFullScreen(true);
        //swords
        Image link_sword_down = new Image("file:src\\game\\oldSprites\\link_sword_down.png");
        Image link_sword_left = new Image("file:src\\game\\oldSprites\\link_sword_left.png");
        Image link_sword_right = new Image("file:src\\game\\oldSprites\\link_sword_right.png");
        Image link_sword_up = new Image("file:src\\game\\oldSprites\\link_sword_up.png");
        //staff / magic
        Image link_staff_down = new Image("file:src\\game\\oldSprites\\link_staff_down.png");
        Image link_staff_left = new Image("file:src\\game\\oldSprites\\link_staff_left.png");
        Image link_staff_right = new Image("file:src\\game\\oldSprites\\link_staff_right.png");
        Image link_staff_up = new Image("file:src\\game\\oldSprites\\link_staff_up.png");
        //running 1
        Image link_running_down_1 = new Image("file:src\\game\\oldSprites\\link_running_down_1.png");
        Image link_running_left_1 = new Image("file:src\\game\\oldSprites\\link_running_left_1.png");
        Image link_running_up_1 = new Image("file:src\\game\\oldSprites\\link_running_up_1.png");
        Image link_running_right_1 = new Image("file:src\\game\\oldSprites\\link_running_right_1.png");
        //running 2
        Image link_running_down_2 = new Image("file:src\\game\\oldSprites\\link_running_down_2.png");
        Image link_running_left_2 = new Image("file:src\\game\\oldSprites\\link_running_left_2.png");
        Image link_running_up_2 = new Image("file:src\\game\\oldSprites\\link_running_up_2.png");
        Image link_running_right_2 = new Image("file:src\\game\\oldSprites\\link_running_right_2.png");
        //map indicator - shows what room the player is in
        ImageView indicator = new ImageView("file:src\\game\\Indicator.png");
        InputManager(mapPieces, backgrounds, items, enemies, magic, fireballs, inventory, inventoryARR, animationHero, itemName, itemType, itemDescription, itemEffect, itemId, group, scene, translate, bossIMG, hero, indicator, map, stage, northWall, eastWall, southWall, westWall, link_sword_down, link_sword_left, link_sword_right, link_sword_up, link_staff_down, link_staff_left, link_staff_right, link_staff_up, sword_down, sword_left, sword_right, sword_up, usedSword, bossDoor_closed, bossDoor_open, bossDoor, chest, chest_open, controls, bossHealthBar, bossHealthBarREC, scoreBoard, hearts, inventoryPIC, selected, score, coinNumber, scoreboardLabels, northDoor, eastDoor, southDoor, westDoor, background, link_running_down_1, link_running_left_1, link_running_right_1, link_running_up_1, shopkeeper, link_running_down_2, link_running_left_2, link_running_right_2, link_running_up_2);
        ...
        stage.setOnCloseRequest(event -> System.out.println("closed"));
        int i = 0;
    ...
    private static void labelMaker(Label label, String text, int X, int Y, int fontSize, Color fontColor, boolean Visible, double opacity) {
        //labelMaker(coinNumber, String.valueOf(CoinNumber), 50, 98, 21, Color.WHITE, true, 1);
        // template ^
    ...
    private void bossRoom(int[][] inventoryARR, ArrayList<MapPieces> mapPieces, ArrayList<Backgrounds> backgrounds, ArrayList<Items> items, ArrayList<Enemy> enemies, ArrayList<Magic> magic, ArrayList<Fireballs> fireballs, ArrayList<Inventory> inventory, Group group, Scene scene, ImageView chest, ImageView bossHealthBar, Rectangle bossHealthBarREC, ImageView hearts, Label score, ImageView hero, ImageView boss, Stage stage, int[][] map, ImageView indicator, ImageView northDoor, ImageView eastDoor, ImageView southDoor, ImageView westDoor, ImageView bossDoor, Image bossDoor_closed, Image bossDoor_open, ImageView background, TranslateTransition translate) {
        ...
        Timeline fireballDetector = null;
        bossARR.add(new Boss(group, fireballs, hero, hero_Health,chest,bossARR,fireballDetector));
        if (bossARR.get(0).getName().equalsIgnoreCase("Wizard")) {
            fireballDetector = new Timeline(new KeyFrame(Duration.seconds(0.05), event -> {
                for (int m = 0; m < fireballs.size(); m++) {
                    if (fireballs.get(m).getImage().getBoundsInParent().intersects(hero.getBoundsInParent())) {
            ...
            fireballDetector.setCycleCount(Timeline.INDEFINITE);
            fireballDetector.play();
        }
    ...
    private void ScoreDisplayer(Group group, ImageView scoreBoard, Label[][] ScoreBoard) {
        ...
        //TODO NEW DESKTOP IS: DESKTOP-7M8HTIQ - sql database is not set up
    ...
    private void AnimationManager(ImageView hero, String animation) {
        ...
        if (i == 0) {
            ...
            Timeline animationTimeline = new Timeline(new KeyFrame(Duration.seconds(0.35), event -> {
                ...
                if (!(direction.equals(""))) {
                    if (anim[0].equals("b")) {
                        Image image1 = new Image("file:src\\game\\oldSprites\\link_running_" + direction + "_1.png");
                        hero.setImage(image1);
                        anim[0] = "a";
                    } else {
                        Image image2 = new Image("file:src\\game\\oldSprites\\link_running_" + direction + "_2.png");
...
/*
                        for (int m = 0; m < items.size(); m++) {
                            if (items.get(m).getState().equalsIgnoreCase("shop")) {
                                items.get(m).getImage().setVisible(true);
                                items.get(m).getLabel().setVisible(true);
                                items.get(m).getCoinIcon().setVisible(true);
                                //System.out.println(items.get(m).getLabel().getText());
                            }
                        }
...
/*
                        for (int m = 0; m < items.size(); m++) {
                            if (items.get(m).getState().equalsIgnoreCase("shop")) {
                                items.get(m).getImage().setVisible(false);
                                items.get(m).getLabel().setVisible(false);
                                items.get(m).getCoinIcon().setVisible(false);
...
*/
                    ...
                    case D:
                        ...
                        break;
                    case E:
                        itemInteraction(mapPieces, backgrounds, items, enemies, magic, fireballs, inventory, inventoryARR, itemId, group, scene, hero, indicator, chest, chest_open, hearts, inventoryPic, selected, coinNumber, Labels, stage, score, itemName, itemType, itemDescription, itemEffect, map, northDoor, eastDoor, southDoor, westDoor, bossDoor, bossDoor_closed, bossDoor_open, background, translate, bossHealthBar, bossHealthBarREC, boss);
                    ...
                    case I:
                        ...
                        //hides the inventory controls
                        controls.setVisible(!controls.isVisible());//shows the inventory controls
                        break;
                    case M:
    ...
    private void MusicControls() {
        if (!activeBoss) {                        //decides if the background music or boss music is playing
            MEDIABackground.setMute(!MEDIABackground.isMute());
        } else {
            MEDIABoss.setMute(!MEDIABoss.isMute());
        }
    }
    ...
    private void itemInteraction(ArrayList<MapPieces> mapPieces, ArrayList<Backgrounds> backgrounds, ArrayList<Items> items, ArrayList<Enemy> enemies, ArrayList<Magic> magic, ArrayList<Fireballs> fireballs, ArrayList<Inventory> inventory, int[][] inventoryARR, int itemId, Group group, Scene scene, ImageView hero, ImageView indicator, ImageView chest, Image chest_open, ImageView hearts, ImageView inventoryPIC, ImageView selected, Label coinNumber, Label[][] Labels, Stage stage, Label score, Label itemName, Label itemType, Label itemDescription, Label itemEffect, int[][] map, ImageView northDoor, ImageView eastDoor, ImageView southDoor, ImageView westDoor, ImageView bossDoor, Image bossDoor_closed, Image bossDoor_open, ImageView background, TranslateTransition translate, ImageView bossHealthBar, Rectangle bossHealthBarREC, ImageView boss) {
/*
        ...
        */
        if (chest.isVisible() && hero.getBoundsInParent().intersects(chest.getBoundsInParent())) {
            if (f == 0) {
                ...
                Image link_holding_triforce = new Image("file:src\\game\\oldSprites\\link_holding_triforce.png");
        ...
        if (!inventoryPIC.isVisible()) {
            //picking up an item/imageViewIMG off of the floor
            for (Items image : items) {
                if (hero.getBoundsInParent().intersects(image.getImage().getBoundsInParent()) && image.getImage().getOpacity() > 0 && image.getImage().isVisible()) {//checks if you're touching any potions
                    if (image.getState().equalsIgnoreCase("shop")) {
                        if (CoinNumber >= image.getPrice()) {
                            switch (image.getName()) {
                                ...
                                case "attack boost":
                                    activeAttackBoost = true;
                                    inventory.add(new Inventory(image.getID()));
                                    for (int m = 0; m < 6; m++) {
                                        for (int o = 0; o < 5; o++) {
                                            if (inventoryARR[o][m] == 0) {//finds the first free slot in the inventory
                                                inventory.get(inventory.size() - 1).getImage().relocate((643 + (68 * (o))), 314 + (67 * (m)));
                            ...
                            image.getCoinIcon().setVisible(false);
                            CoinNumber = (CoinNumber - image.getPrice());
                            coinNumber.setText(String.valueOf(CoinNumber));
                            new Pulse(hero).play();
                        }
                    } else if (image.getState().equalsIgnoreCase("random")) {
        ...
        if (anInventory.getImage().getOpacity() > 0 && anInventory.getImage().isVisible()) {
            ...
            switch (relativeItemName) {
                ...
                case "Attack boost"://uses an attack boost
                    anInventory.getImage().setOpacity(0);
                    group.getChildren().remove(anInventory.getImage());
                    attack = (int) (attack * 1.25);//TODO possible trip up
                    System.out.println(attack);
                    System.out.println("attack is now " + attack);
                    break;
            }
            itemInformation(inventory, selected, itemName, itemType, itemDescription, itemEffect);
        }
                    ...
                    try {
                        if (!activeBoss) {
                            ...
                            if (magic.get(0).getImage().getBoundsInParent().intersects(boss.getBoundsInParent()) && boss.isVisible()) {//when you hit the boss
                                ...
                                bossHealthBarREC.setWidth(658 - (((65.8) / 2 * y)) * ((attack / 5) * attackmultiplier));
    ...
    private void attackingEnemies(ArrayList<Enemy> enemies, Label score, TranslateTransition translate, ImageView boss, Rectangle sword_down, Rectangle sword_left, Rectangle sword_right, Rectangle sword_up, Rectangle usedSword, Image bossDoor_closed, Image bossDoor_open, ImageView bossDoor, ImageView bossHealthBar, Rectangle bossHealthBarREC, Label coinNumber) {
        try {
            if (bossARR.size()>0) {//attacking the boss
                if (timePassed >= 1 && usedSword.getBoundsInParent().intersects(bossARR.get(0).getImageViewIMG().getBoundsInParent())) {
                    timePassed = 0;
                    System.out.println("hit boss");
                    bossARR.get(0).setHealth(bossARR.get(0).getHealth()-attack);
    ...
    private void roomChanger(int[][] inventoryARR, ArrayList<MapPieces> mapPieces, ArrayList<Backgrounds> backgrounds, ArrayList<Items> items, ArrayList<Enemy> enemies, ArrayList<Magic> magic, ArrayList<Fireballs> fireballs, ArrayList<Inventory> inventory, Group group, Scene scene, TranslateTransition translate, ImageView boss, ImageView indicator, int[][] map, ImageView hearts, ImageView northDoor, ImageView eastDoor, ImageView southDoor, ImageView westDoor, ImageView bossDoor, Image bossDoor_closed, Image bossDoor_open, ImageView chest, ImageView bossHealthBar, Rectangle bossHealthBarREC, Label score, ImageView background, ImageView shopkeeper, ImageView hero, Stage stage) {
        ...
        if (hero.getBoundsInParent().intersects(northDoor.getBoundsInParent()) && northDoor.isVisible() && Objects.equals(direction, "up")) {
            ...
            indicator.relocate(indicator.getLayoutX(), indicator.getLayoutY() - 64);
            hero.relocate((960) - 75, 750);
            Base_Procedure2(inventoryARR, mapPieces, backgrounds, items, enemies, magic, fireballs, inventory, score, translate, hero, map, northDoor, eastDoor, southDoor, westDoor, bossDoor, bossDoor_closed, bossDoor_open, background, hearts, shopkeeper, group, scene, stage, indicator, bossHealthBar, bossHealthBarREC, boss);
        }
    ...
    private void doorChanger(int[][] map, ImageView northDoor, ImageView eastDoor, ImageView southDoor, ImageView westDoor, ImageView bossDoor) {
        //checks above
        northDoor.setVisible(map[positionY - 1][positionX] == 1 || map[positionY - 1][positionX] == 2);
        //checks above for the boss room
        bossDoor.setVisible(map[positionY - 1][positionX] == 3);
        //checks below
        southDoor.setVisible((map[positionY + 1][positionX] == 1) || map[positionY + 1][positionX] == 2);
        //checks right
        eastDoor.setVisible((map[positionY][positionX + 1] == 1) || map[positionY][positionX + 1] == 2);
        //checks left
        westDoor.setVisible((map[positionY][positionX - 1] == 1) || map[positionY][positionX - 1] == 2);
    }

    private static void enemyChanger(ArrayList<Enemy> enemies) {
        for (Enemy enemy : enemies) {
            enemy.getImage().setVisible(positionX == enemy.getRoomX() && positionY == enemy.getRoomY());//changes the visible enemy
        }
    }

    private static void shopChecker(ArrayList<Enemy> enemies, int[][] map, ImageView shopkeeper, ArrayList<Items> items) {
        if (map[positionY][positionX] == 2) {//checks if the current room is a shop
            ...
            for (Items item : items) {
                if (item.getState().equalsIgnoreCase("shop")) {
                    item.getImage().setVisible(true);
                    item.getLabel().setVisible(true);
                    item.getCoinIcon().setVisible(true);
            ...
            for (Items item : items) {
                if (item.getState().equalsIgnoreCase("shop")) {
                    ...
                    item.getCoinIcon().setVisible(false);
    ...
    private static void itemChanger(ArrayList<Items> items) {
        for (Items item : items) {
            item.getImage().setVisible(positionX == item.getRoomX() && positionY == item.getRoomY() && item.getState().equalsIgnoreCase("random"));//changes the visible imageViewIMG
        }
    }
    ...
    private static void mapMaker(ArrayList<MapPieces> mapPieces, ArrayList<Backgrounds> backgrounds, ArrayList<Items> items, ArrayList<Enemy> enemies, Group group, int[][] map) {
        ...
        do {//do while repeats if a map is made without shop or boss room
            ...
            for (int i = 0; i < 19; i++) {
                for (int j = 0; j < 19; j++) {
                    if (map[i][j] == 1) {
                        ...
                        items.add(new Items(i, j, "random", 0));//creates a new instance of an item
                        localRooms++;
                    }
                    ...
                    if (map[i][j] == 1 && k == 1 && map[i - 1][j] == 0 && localRooms > 3) {//creates a shop room, after the boss room is created.
                        map[i - 1][j] = 2;
                        for (int l = 0; l < 3; l++) {
                            items.add(new Items(i - 1, j, "shop", l));
                        }
                        shop = true;
                        k = 2;
                        localRooms++;
            ...
            for (Items item : items) {
                ...
                if (!(item.getLabel() == null && item.getCoinIcon() == null)) {//sets up the labels and coin icons for shop items
                    //checks if the items have a Label and icon attatched, if they do...

                    Text text = new Text(item.getLabel().getText());
//        labelMaker(coinNumber, String.valueOf(CoinNumber), 50, 98, 21, Color.WHITE, true, 1);
                    labelMaker(item.getLabel(), String.valueOf(item.getPrice()), 0, 0, 21, Color.WHITE, true, 1);
                    item.getLabel().relocate(item.getLayoutX() + (item.getImage().getBoundsInParent().getWidth() / 2) - (2 * (text.getLayoutBounds().getWidth())), item.getLayoutY() + (item.getImage().getBoundsInParent().getHeight()));
                    //System.out.println(text.getLayoutBounds());
                    //System.out.println(item.getLabel());
                    item.getLabel().setVisible(false);
                    group.getChildren().add(item.getLabel());


                    item.getCoinIcon().relocate(item.getLabel().getLayoutX() + (2 * (text.getLayoutBounds().getWidth())), item.getLabel().getLayoutY() + 4);
                    item.getCoinIcon().setVisible(false);
                    group.getChildren().add(item.getCoinIcon());

                }
                //System.out.println(item.getCoinIcon());
